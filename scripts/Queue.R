# Adaptación del programa de Law y Kelton para modelo M/M/1
...
# En relación a los indicadores del sistema
  ...
  # area_status_servidor = area_server_status
  # q_t                  = longitud de la fila en un instante dado.
  
  # num_eventos = num_events = numero de eventos para la función de tiempo
  
  # clientes_enespera     = clientes que han esperado en fila
  # Qlim                  = límite de la longitud de la fila
...
mm1 <- function(lambdaA = 15, lambdaS = 10, n = 100, Qlim = 1000){
    inicializa(lambdaA = lambdaA, lambdaS = lambdaS, n = n, Qlim = Qlim) #inicializa simulación
...
# tiempo_ultimo_evento = tiempo del evento más reciente
inicializa  <- function(lambdaA, lambdaS, n, Qlim){
  ...
  Qlim     <<- Qlim           # límite de la fila
  num_eventos <<- 2       # número de eventos distintos (llegadas y salidas)
  lt_A     <<- vector(mode="numeric", length = Qlim + 1)
...
#Función de reporte
reporte <- function(){
  print(paste("Promedio de espera en la fila:", round(total_esperas/clientes_enespera, 2), sep=" "))
  print(paste("Número promedio de clientes esperando en la fila:",round(area_num_q/reloj, 2), sep = " "))
  ...
  return(list(promedio.espera = total_esperas/clientes_enespera,
              longitud.promedio.fila = area_num_q/reloj,
...
actualiza_estadisticas <- function(){
  ...
  area_num_q <<- area_num_q + q_t * tiempo_desde_ultimo_evento
